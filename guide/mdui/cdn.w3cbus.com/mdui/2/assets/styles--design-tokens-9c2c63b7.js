const d={docsHtml:`<p>Design Tokens are a strategy employed in managing design systems.</p>
<p>They abstract reusable elements such as colors, fonts, and spacing into independent variables. These variables are then consistently used across the design system.</p>
<style>
.design-tokens-color {
...
<p>mdui employs global CSS custom properties to implement Design Tokens, ensuring style consistency across all components. By modifying these properties, you can globally adjust the styles of all mdui components. For your own styles, prioritize using these CSS custom properties to maintain consistency with mdui component styles. This also ensures your styles update synchronously when modifying dynamic color schemes.</p>
<h2 id="color">
                  <a href="#color">Color</a>
                </h2>
<p>mdui provides CSS custom properties for both light and dark modes. In light mode, the property is named <code>--mdui-color-{name}-light</code>, and in dark mode, it&#39;s <code>--mdui-color-{name}-dark</code>.</p>
<p>Additionally, mdui provides CSS custom properties named <code>--mdui-color-{name}</code>. This property references <code>--mdui-color-{name}-light</code> in light mode and <code>--mdui-color-{name}-dark</code> in dark mode, enabling automatic color switching based on the current mode.</p>
<p>To modify specific colors, adjust both <code>--mdui-color-{name}-light</code> and <code>--mdui-color-{name}-dark</code>. When reading CSS custom properties, use the <code>--mdui-color-{name}</code> property.</p>
<p>The values of CSS custom properties are three RGB colors separated by <code>,</code>. Here&#39;s an example of how to use color CSS custom properties:</p>
<pre><code class="language-css">/* Modify the color value of primary */
:root {
...
<p>For creating a completely new color scheme, consider using the <a href="https://cdn.w3cbus.com/en/docs/2/functions/setColorScheme"><code>setColorScheme</code></a> function. This function generates a complete color scheme based on the provided color.</p>
<table>
  <thead>
    <tr>
      <th>Color Name</th>
...
<h2 id="shape-corner">
                  <a href="#shape-corner">Corner Radius</a>
                </h2>
<p>mdui provides 7 corner radius sizes. These CSS custom properties allow you to adjust the corner radius:</p>
<pre><code class="language-css">/* Modify the corner radius size of extra-smal */
:root {
...
<h2 id="typescale">
                  <a href="#typescale">Typography</a>
                </h2>
<p>mdui provides 15 typography styles. Each style includes properties for line height, font size, letter spacing, and font weight.</p>
<p>Here&#39;s an example of how to use these properties:</p>
<pre><code class="language-css">/* Modify the text style of Body large */
:root {
...
<h2 id="state-layer">
                  <a href="#state-layer">State Layer Opacity</a>
                </h2>
<p>mdui components, such as <a href="https://cdn.w3cbus.com/en/docs/2/components/button"><code>&lt;mdui-button&gt;</code></a>, utilize a semi-transparent overlay layer during interaction states like hover, focus, press, and drag. The opacity of this layer can be adjusted by modifying the following CSS custom properties:</p>
<pre><code class="language-css">/* Modify the opacity of the state layer */
:root {
...
<p>mdui components use elevation to create depth with shadows. You can adjust these shadows by modifying the CSS custom properties:</p>
<pre><code class="language-css">/* Modify the elevation of level1 */
:root {
...
<p>mdui components incorporate animations, with customizable easing curves and durations. These properties can be adjusted using CSS custom properties:</p>
<pre><code class="language-css">/* Modify the standard easing curve and short1 duration */
:root {
...
<p>mdui components adjust their layout based on various breakpoints, which are provided through CSS custom properties. Here&#39;s an example of how to modify a breakpoint:</p>
<pre><code class="language-css">/* Modify the breakpoint value for sm */
:root {
  --mdui-breakpoint-sm: 560px;
}
</code></pre>
<p>Please note that CSS custom properties cannot be used in CSS media queries. For example, the following usage is incorrect:</p>
<pre><code class="language-css">/* Incorrect usage. CSS custom properties cannot be used in media queries */
@media (min-width: var(--mdui-breakpoint-sm)) {

}
</code></pre>
<p>To determine breakpoints in JavaScript, use the <a href="https://cdn.w3cbus.com/en/docs/2/functions/breakpoint"><code>breakpoint</code></a> function.</p>
<table>
<thead>
<tr>
<th>CSS Custom Property</th>
