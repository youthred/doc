const e={docsHtml:`<p>范围滑块组件用于让用户在一系列值中选择一个范围。</p>
<h2 id="usage">
                  <a href="#usage">使用方法</a>
                </h2>
<p>按需导入组件：</p>
...
<p>通过 <code>value</code> 属性，可以读取或设置范围滑块的当前值。该属性值是一个数组，只能通过 JavaScript 属性进行读取和设置。</p>
...
<p>添加 <code>disabled</code> 属性可以禁用范围滑块。</p>
...
<p>使用 <code>min</code> 和 <code>max</code> 属性设置范围滑块的最小值和最大值。</p>
...
<p>使用 <code>step</code> 属性设置范围滑块的步进间隔。</p>
...
<p>添加 <code>tickmarks</code> 属性可以在范围滑块上添加刻度标记。</p>
...
<p>添加 <code>nolabel</code> 属性可以隐藏范围滑块上的文本提示。</p>
...
<p>通过 <code>labelFormatter</code> JavaScript 属性，可以修改文本提示的显示格式。该属性值是一个函数，函数参数为当前范围滑块的值，返回值为你期望显示的文本。</p>
...
`,elements:[{name:"RangeSlider",tagName:"mdui-range-slider",summary:`<p>范围滑块组件</p>
<pre><code class="language-html">&lt;mdui-range-slider&gt;&lt;/mdui-range-slider&gt;
</code></pre>
`,attributes:[{propertyName:"defaultValue",reflect:!1,description:`<p>默认值。在重置表单时，将重置为该默认值。此属性只能通过 JavaScript 属性设置</p>
`,type:"number[]",enum:[],default:"[]"},{propertyName:"value",reflect:!1,description:`<p>滑块的值，为数组格式，将于表单数据一起提交。</p>
...
`},{name:"change",description:`<p>值发生变更，且失去焦点时，将触发该事件</p>
